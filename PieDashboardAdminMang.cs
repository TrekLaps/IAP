...
public partial class PieDashboardAdminMang : System.Web.UI.Page
{
    ...
    protected void MainYear_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("PieDashboardAdminMang.aspx?ReqY=" + MainYear.SelectedValue + "&S=" + Request.QueryString["S"] + "&Reqq=" + Request.QueryString["Reqq"]);

        }
        else
        {
            Response.Redirect("PieDashboardAdminMang.aspx?ReqY=0&S=" + Request.QueryString["S"] + "&Reqq=" + Request.QueryString["Reqq"]);
    ...
    private void LoadDataChart()
    {
        ...
        if (Session["UData"] != null)
        {
            ...
            MainTitle.InnerHtml = "<h3>" + Case + "</h3>";
...
dataImport.addColumn({type: 'string', role: 'tooltip', 'p': {'html': true}});
            ...
            strImport.Append(" chartImport.draw(dataImport,{'title': '',pieHole: 0.4, 'width': 420, 'height': 300,tooltip: {isHtml: true}, 'backgroundColor': 'transparent',chartArea:{left:2,top:10,width:'65%',height:'65%'}, 'textAlign': 'center', 'colors': ['#4CAF50', '#FF9800', '#9C27B0'],legend: { position: 'none'}, vAxis: {minValue: 4,viewWindow:{ min: 0},format: '0' }, hAxis: {showTextEvery:1, slantedText:false}});");
  ...
  function selectHandler123() {
            ...
            strImport.Append("self.location = *ReportingAdmMang.aspx?S=" + Request.QueryString["S"] + "&Reqq=" + Request.QueryString["Reqq"] + "&YQ=" + Plan + "&qq=*+ chartImport.getSelection()[0].row;}}} ");
...
dataImport.addColumn({type: 'string', role: 'tooltip', 'p': {'html': true}});
  ...
  function selectHandler123() {
            ...
            strImport.Append("self.location = *ReportingAdmMang.aspx?S=" + Request.QueryString["S"] + "&Reqq=" + Request.QueryString["Reqq"] + "&qq=*+ chartImport.getSelection()[0].row;}}} ");
    ...
    private void BindStatChartAdmin(int Admin, int Plan)
    {
        ...
        try
        {
            ...
            strStat.Append(" chartStat.draw(dataStat,{width:420, height:300,pieHole: 0.4,'textAlign': 'right',chartArea:{left:2,top:10,width:'65%',height:'65%'},legend: {position: 'none' }, vAxis: {minValue: 4,viewWindow:{ min: 0},format: '0' },'colors': ['#93cdf3','#727aa2','#9C27B0','#FF9800','#4CAF50'], hAxis: {showTextEvery:1, slantedText:false}});");
