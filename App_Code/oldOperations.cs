...
";
    ...
    public string ReadRepwithStatusAllImp = @"select dbo.Reports.RepID ,dbo.Reports.RepText,RepStatus,dbo.Reports.Importance ,RepTitle
FROM            dbo.Reports INNER JOIN
...
WHERE         (dbo.Reports.Active = 1)";
    ...
    public DataSet SignInByUserName(string UserName)
    {
        try
        {
            ...
            SqlDataAdapter ad = new SqlDataAdapter();
            ConOpen();
            CMD.Parameters.Add("@EmpJobCode", UserName); //Username (Job Code orEmail)
    ...
    public void OurMails(string Subject, string Body, int SectionHead, int Admin, bool Reply)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    DataSet DsSection = GetDataSetByID("GetSectionHeadMail", SectionHead);
                    objeto_mail.To.Add(new MailAddress(Convert.ToString(DsSection.Tables[0].Rows[0]["EmpEmail"])));
                ...
                if (Admin != 0 && Admin != -1) // Report or Note
                {
                    ...
                    DataSet DsAll = GetDataSetByID("GetRecivEmails", Admin);
