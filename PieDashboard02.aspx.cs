...
public partial class PieDashboard02 : System.Web.UI.Page
{
    ...
    private void LoadDataChart()
    {
        if (hidden.Value != "0")
        {
            ...
            //RepImpData.DataSource = Obj.GetChartBySectionAdminStat(Convert.ToInt32(Admins.SelectedValue), Obj.ExecuteProcedureString("GetAdminByName", HiddenTitle.Value), Stats);
            ...
            RepImpData.DataSource = Obj.GetChartBySectionAdminPlanStat(Convert.ToInt32(Request.QueryString["ReqYR"]), Convert.ToInt32(Admins.SelectedValue), Obj.ExecuteProcedureString("GetAdminByName", HiddenTitle.Value), Stats);
    ...
    private DataTable GetData()
    {
        ...
        cmd.Parameters.AddWithValue("@ID", Admins.SelectedValue);
        cmd.Connection = Obj.Conn;
    ...
    private void BindChart()
    {
        ...
        try
        {
           ...
           // dt = GetData();
            ...
            if (string.IsNullOrEmpty(DropYear.SelectedValue) || DropYear.SelectedValue == "0")
                dt = GetData();
            else
                dt = GetDataPlan(Convert.ToInt32(DropYear.SelectedValue), Convert.ToInt32(Admins.SelectedValue));
...
message = 'nothing';
    ...
    private DataTable GetDataImport0(int Plan)
    {
        ...
        cmd.Parameters.AddWithValue("@Section", Convert.ToInt32(Admins.SelectedValue));
        cmd.Connection = Obj.Conn;
    ...
    private void BindImportChart()
    {
        ...
        try
        {
            if (string.IsNullOrEmpty(DropYear.SelectedValue) || DropYear.SelectedValue == "0")
                ...
                dtImport = GetDataImportPlan(Convert.ToInt32(DropYear.SelectedValue), Convert.ToInt32(Admins.SelectedValue));
...
dataImport.addColumn({type: 'string', role: 'tooltip', 'p': {'html': true}});
            ...
            for (int i = 0; i <= dtImport.Rows.Count - 1; i++)
            {
                ...
                DataSet DSCounts = Obj.GetDataSetBy3ID("GetRepCountByStatSec01NEW", Convert.ToInt32(dtImport.Rows[i]["Import"].ToString()), Convert.ToInt32(Request.QueryString["Reqq"]), Convert.ToInt32(Request.QueryString["ReqYR"]));
            ...
            strImport.Append(" chartImport.draw(dataImport,{'title': '', 'width': 320,pieHole: 0.4, 'height': 320,tooltip: {isHtml: true}, 'backgroundColor': 'transparent',chartArea:{left:2,top:7,width:'85%',height:'85%'}, 'textAlign': 'center', 'colors': ['#4CAF50', '#FF9800', '#9C27B0'],legend: { position: 'none'}, vAxis:{minValue: 4,viewWindow:{ min: 0},format: '0' }, hAxis: {showTextEvery:1, slantedText:false}});");
  ...
  function selectHandler123() {
                    ...
                    if (selectedItem) {");
            strImport.Append("self.location = *ReportingSection.aspx?ReqYR="+Request.QueryString["ReqYR"]+"&Reqq="+Request.QueryString["Reqq"]+"&Imp=*+ chartImport.getSelection()[0].row;}}}");
    ...
    private void BindStatChart()
    {
        ...
        try
        {
            ...
            //dtStat = GetDataStat();
           if ((string.IsNullOrEmpty(DropYear.SelectedValue) || DropYear.SelectedValue == "0") &&(Admins.SelectedValue!="" || Admins.SelectedValue!=null || Admins.SelectedValue!="0"))
                dtStat = GetDataStat();
            else
                dtStat = GetDataStatPlan(Convert.ToInt32(DropYear.SelectedValue), Convert.ToInt32(Admins.SelectedValue));
            ...
            /*التعديل هنا*/
            strStat.Append(" chartStat.draw(dataStat,{width:320, height:320,pieHole: 0.4,'textAlign': 'right',chartArea:{left:2,top:7,width:'85%',height:'85%'},legend: {position: 'none' }, vAxis:{minValue: 4,viewWindow:{ min: 0},format: '0' },'colors': ['#93cdf3','#727aa2','#9C27B0','#FF9800','#4CAF50'], hAxis: {showTextEvery:1, slantedText:false}});}");
            /*-----*/
    ...
    {
        Response.Redirect("SectionAdminsCharts.aspx?ReqYR="+Request.QueryString["ReqYR"] + "&Reqq="+Request.QueryString["Reqq"]);
    }

    protected void DropYear_SelectedIndexChanged(object sender, EventArgs e)
    {
       
            Response.Redirect("PieDashboard02.aspx?Reqq="+Admins.SelectedValue+"&ReqYR="+DropYear.SelectedValue);
    ...
    protected void Admins_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("PieDashboard02.aspx");
        }
