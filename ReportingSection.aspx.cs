...
public partial class ReportingSection : System.Web.UI.Page
{
    ...
    {
        Int32 Imp = Convert.ToInt32(Request.QueryString["Imp"]) + 1;

        if (!IsPostBack)
        {
            if (Request.QueryString["Imp"] != null)
            {
                if (Request.QueryString["Imp"] == "0")
                {
    ...
    protected void BackCharts_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("PieDashboardSection.aspx?ReqY=0&Reqq=" + Convert.ToString(Request.QueryString["Reqq"]));
        }
        else if (Convert.ToBoolean(MyRecDataSet.Tables[0].Rows[0]["ApprovPermission"]) == true)
        {
            Response.Redirect("PieDashboard02.aspx?ReqYR=" + Convert.ToString(Request.QueryString["ReqYR"]) + "&Reqq=" + Convert.ToString(Request.QueryString["Reqq"]));
        }
        else
        {
