...
public partial class ManagerReports : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["UData"] != null)
        {
            ...
            if (Obj.ExecuteProcedureID("CheckAdminManger", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["EmpID"])) == 1 || Obj.ExecuteProcedureID("CheckSectionManger", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["EmpID"])) == 1)
            {
                if (!IsPostBack)
                {  // Fill dropdown Lists For Reports with date and RepiD
                    ...
                    Admins1.DataBind();
                    DataSet DsFiles = Obj.GetDataSetByID("GetAdminFilesByAdmin", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["AdmID"]));
                    ...
                    Adm.InnerText = Convert.ToString(MyRecDataSet.Tables[0].Rows[0]["AdmName"]);


                    BindEmployeesData();
                    DataTable DTImportant = Obj.GetDataSetByID("ReportCountImportByAdm", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["AdmID"])).Tables[0];

                    DataTable DTStatus = Obj.GetDataSetByID("ReportCountStatusByAdm", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["AdmID"])).Tables[0];
    ...
    protected void Admins1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (Admins1.SelectedValue != "" && Admins1.SelectedValue != "0")
        {
            CheckRepeat.Checked = false;
        ...
        else if (Admins1.SelectedValue == "0")
        {
            if (Session["UData"] != null)
            {
                DataSet MyRecDataSet = (DataSet)Session["UData"];
    ...
    protected void RepNotes_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        DataSet DsFiles = Obj.GetDataSetByID("GetNoteFiles", Convert.ToInt32(((Label)e.Item.FindControl("LblRepp")).Text));
        ...
        else if (((Label)e.Item.FindControl("LblNoteStat")).Text == "5")
        {
            ...
            ((Label)e.Item.FindControl("LblNoteStat")).Text = "مغلقة";
            td2.Attributes.Add("style", "background-color:#cccccc;");
        }
