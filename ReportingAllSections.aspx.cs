...
public partial class ReportingAllSections : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["UData"] != null)
            {
                ...
                if (Request.QueryString["RAdm"] != null)
                {
                    for (int i = 0; i <= AdminList.Items.Count - 1; i++)
                    {
                        if (AdminList.Items[i].Value == Request.QueryString["RAdm"])
                        {
                            AdminList.Items[i].Selected = true;
                        }
                    }
                }
	  ...
	  when " + DropImport.SelectedValue + @" = 0 then dbo.Reports.Importance
                ...
                else if (Request.QueryString["ReqY"] != null && Request.QueryString["ReqY"] != "0")
    ...
    protected void BackCharts_Click(object sender, EventArgs e)
    {
            ...
            if (Request.QueryString["ReqY"] != null && Request.QueryString["ReqY"] != "")
            {
                Response.Redirect("PieDashboardSection.aspx?ReqYR=" + Request.QueryString["ReqY"] + "&Reqq=" + Request.QueryString["Reqq"]);
            }
            else
            {

                Response.Redirect("PieDashboardSection.aspx?ReqY=0&Reqq=" + Request.QueryString["Reqq"]);
