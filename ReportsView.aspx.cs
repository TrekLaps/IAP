...
public partial class ReportsView : System.Web.UI.Page
{
    ...
    private void BindEmployeesData()
    {
        if (Session["UData"] != null)
        {
            if (Session["UData"] != null)
            {
                ...
                EmployeesData.DataSource = Obj.GetDataSetByID("GetReportByAdm", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["AdmID"]));
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["UData"] != null)
                {
                    ...
                    if ((Obj.ExecuteProcedureID("CheckAdminManger", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["EmpID"])) == 1) || Convert.ToBoolean(MyRecDataSet.Tables[0].Rows[0]["Reciv"]) == true)
                    {
                        /// Log Data Start
                        Obj.ExecuteProcedureStringID("NewLogTable", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["EmpID"]), "View Notes and Recommendations by Department manager permission");
                        /// Log Data End
                        BindEmployeesData();
                        LblUpdate.Text = "";

                        // Fill dropdown Lists For Reports with date and RepiD
                        Admins1.DataSource = Obj.GetDataSetByID("GetReportMainByAdmin", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["AdmID"]));
                        ...
                        // Fill dropdown Lists For Reports with date and RepiD
                        PlansSearch.DataSource = Obj.GetDataSetByID("GetPlansByAdmin", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["AdmID"]));
            ...
            else
            {
                ...
                if (HiddenPost.Value == "Log")
                {
                    if (HiddenLogText.Value.Length > 0)
                    {
                        ...
                        Obj.ExecuteProcedureStringID("NewLogTable", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["EmpID"]), "Read Recommendation No." + arg[1] + " for Note No." + arg[0] + "As Department Manager Permission ");
    ...
    protected void LinkDetails_Command(object sender, CommandEventArgs e)
    {
        ...
        Obj.ExecuteProcedureStringID("NewLogTable", Convert.ToInt32(MyRecDataSet.Tables[0].Rows[0]["EmpID"]), "Read Note No." + LblNo.Text + "As Department Manager Permission ");
    ...
    protected void AddFile_Click(object sender, EventArgs e)
    {
        if (Session["UData"] != null)
        {
            ...
            if (FileUploadR.FileName.Length > 0)
            {
                ...
                var Res = Obj.NewAttachCommentNote(Convert.ToInt32(ConfirmId.Text), imgPath, FileUploadR.FileName);
                if (Res != 0)
                {
                    if (Res == -1)
                    {
    ...
    protected void LinkNotes_Command(object sender, CommandEventArgs e)
    {
        ...
        SucReplyNote.Visible = false;
        SucFile.Visible = false;
        editorReply.InnerText = "";
...
" + MyRecDataSet.Tables[0].Rows[0]["EmpName"] + @"
    ...
    ";
            ...
            <b>    للاطلاع على مزيد من التفاصيل اضغط هنا <a href='"+ HttpContext.Current.Request.Url.Authority + @"'>رابط التوصية</a></b>
                                ...
                                Obj.OurMails("رد على التوصية ", Body, Convert.ToInt32(Row["RepSection"]), Convert.ToInt32(Row["RepAdms"]), true);
