...
public partial class ReportingAdmSections : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["UData"] != null)
            {
                ...
                if (Convert.ToBoolean(MyRecDataSet.Tables[0].Rows[0]["ApprovPermission"]) == true || Convert.ToBoolean(MyRecDataSet.Tables[0].Rows[0]["Gov"]) == true || Convert.ToBoolean(MyRecDataSet.Tables[0].Rows[0]["SystemAdmin"]) == true)
                {
                    ...
                    YearList.DataSource = Obj.GetDataSetByID("ReportYearsBySection", Convert.ToInt32(Request.QueryString["Reqq"]));
                    ...
                    if (Request.QueryString["ReqYR"] != null && Request.QueryString["ReqYR"] != "0")
                    {
                        ...
                        {
                            if (YearList.Items[i].Value == Request.QueryString["ReqYR"])
                            {
	  ...
	  when " + DropImport.SelectedValue + @" = 0 then dbo.Reports.Importance
                        ...
                        for (int i = 0; i <= YearList.Items.Count - 1; i++)
                        {
                            ...
                            {
                                sqlwhere += "OR dbo.Plans.ID=" + YearList.Items[i].Value;
                            }
                        }
                        sqlstr += sqlwhere;
                    ...
                    else if (Request.QueryString["ReqYR"] != null && Request.QueryString["ReqYR"] == "0")
    ...
    protected void BackCharts_Click(object sender, EventArgs e)
    {
        if (Request.QueryString["E"] != null)
        {
            ...
            {
                Response.Redirect("MainDashboardGraph02.aspx?Reqq="+ Request.QueryString["ReqYR"]);
            }
            else
            {
        ...
        {
            DataSet MyRecDataSet = (DataSet)Session["UData"];
            Response.Redirect("PieDashboard02.aspx?ReqYR=" + Request.QueryString["ReqYR"] + "&Reqq=" + Convert.ToString(Request.QueryString["Reqq"]));
        }
    }
